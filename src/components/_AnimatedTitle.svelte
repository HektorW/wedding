<script lang="ts">
	...
	const wordDurationInitalMs = 1750
	const wordDurationMs = 4000
	let activeIndex = 0
	...
	onMount(() => {
		let intervalId = setInterval(() => {
			...
			if (activeIndex === 0) {
				...
				intervalId = setInterval(showNextWord, wordDurationMs)
			}
		}, wordDurationInitalMs)
	...
	{#each words as word, index}
		...
		>
			{#each typeof word === 'string' ? Array.from(word) : word as letter, letterIndex}
				<span class="letter" style:--item-easing-t={Math.pow(letterIndex / (word.length - 1), 3)}
